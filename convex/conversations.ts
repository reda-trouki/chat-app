...
export const get = query({
    ...
    handler: async (ctx, args) =>{
       ...
       if(!identity) throw new ConvexError("Unauthorized");
        ...
        await ctx.db.query("conversationsMembers")
        .withIndex("by_meberId", q => q.eq("memberId", currentUser._id))
        .collect();
        ...
        const conversationWithDetails = await Promise.all(conversations.map(async (conversation, index) =>{
            ...
            if(conversation.isGroup) return {conversation, lastMessage, unseenCount: unseenMessages.length}
            else {
